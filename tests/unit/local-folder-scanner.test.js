...
describe('Folder Project Service', () => {
  it('builds a root payload and module list from browser-selected task files', () => {
    ...
    const record = service.__test__.buildProjectRecordFromFileMap({
      ...
      folderIndex: {
        directories: ['src', 'src/SHARED', 'src/SHARED/shared-ui'],
        files: ['README.md', 'node.tasks.json', 'src/node.tasks.json', 'src/SHARED/shared-ui/node.tasks.json']
      },
      fileMap: {
        'node.tasks.json': {
    ...
    expect(record.folderIndex.directories).toContain('src/SHARED/shared-ui');
    expect(record.folderIndex.files).toContain('README.md');
    expect(record.indexedPathCount).toBe(7);
  });

  it('ignores legacy task filenames and keeps discovery scoped to node.tasks.json', () => {
    const { service } = loadFolderProjectService();

    const record = service.__test__.buildProjectRecordFromFileMap({
      folderName: 'legacy-cleanup',
      folderIndex: {
        directories: ['src', 'src/legacy'],
        files: ['node.tasks.json', 'src/node.tasks.json', 'src/legacy/TODO_project_task.json']
      },
      fileMap: {
        'node.tasks.json': {
          project: { name: 'Legacy Cleanup' },
          navigation: { rootModule: 'src/node.tasks.json' }
        },
        'src/node.tasks.json': {
          template_type: 'project_task_template',
          project: { name: 'Legacy Cleanup' },
          tasks: [
            { task_name: 'CLEAN-001: Normalize discovery', status: 'Done', priority: 'High' }
          ]
        },
        'src/legacy/TODO_project_task.json': {
          template_type: 'project_task_template',
          project: { name: 'Legacy Cleanup' },
          tasks: [
            { task_name: 'LEGACY-001: Should be ignored', status: 'Done', priority: 'Low' }
          ]
        }
      }
    });

    expect(record.discoveredFiles).toEqual(['node.tasks.json', 'src/node.tasks.json']);
    expect(record.fileCount).toBe(2);
    expect(record.moduleDataByPath['src/legacy/TODO_project_task.json']).toBeUndefined();
  });
